...
class Program(My_Diary_window.My_Diary_window, My_Diary_interface.My_Diary_interface):
    ...
    def __init__(self: typing.Self, *args, **kwargs) -> None:
        ...
        if locale.getdefaultlocale()[0] == f"sr_RS":
            self.main_screen_new_tab_button: customtkinter.CTkButton = self.main_screen_title_menu_submenu.add_option(option=f"нова картица", command=self.__add_new_tab__)
            self.main_screen_close_tab_button: customtkinter.CTkButton = self.main_screen_title_menu_submenu.add_option(option=f"затвори картицу", command=self.__close_current_tab__)
            ...
            self.main_screen_new_tab_button: customtkinter.CTkButton = self.main_screen_title_menu_submenu.add_option(option=f"новая вкладка", command=self.__add_new_tab__)
            self.main_screen_close_tab_button: customtkinter.CTkButton = self.main_screen_title_menu_submenu.add_option(option=f"закрыть вкладку", command=self.__close_current_tab__)
    ...
    def __word_count_show__(self: typing.Self) -> None:
        ...
        if textbox:
            if locale.getdefaultlocale()[0] == f"sr_RS":
                tkinter.messagebox.showinfo(title=f"речи", message=f"речи: {len(textbox.get(f'1.0', tkinter.END).split())}")
            elif locale.getdefaultlocale()[0] == f"ru_RU":
                tkinter.messagebox.showinfo(title=f"слов", message=f"слов: {len(textbox.get(f'1.0', tkinter.END).split())}")
            else:
    ...
    def __text_summary__(self: typing.Self) -> None:
        textbox = self.__get_current_textbox__()
        if textbox:
            self.summary: str = g4f.ChatCompletion.create(model=f"gpt-4o", messages=[{f"role": f"system", f"content": f"Summarize the following text:"}, {f"role": f"user", f"content": textbox.get(f"1.0", tkinter.END)}])
            if locale.getdefaultlocale()[0] == f"sr_RS":
