...
class Program(customtkinter.CTk):
	...
	def __init__(self, *args, **kwargs) -> None:
		...
		self.main_screen_save_button: customtkinter.CTkButton = customtkinter.CTkButton(master=self.main_screen_taskbar, text="сачувај текст", text_color=text_color, height=20, width=295, corner_radius=5, fg_color=button_color, font=("Roman", 22), command=self.__save__)
		...
		self.main_screen_clear_button: customtkinter.CTkButton = customtkinter.CTkButton(master=self.main_screen_taskbar, text="обриши текст", text_color=text_color, height=20, width=295, corner_radius=5, fg_color=button_color, font=("Roman", 22), command=self.__clear_text__)
		self.main_screen_clear_button.place(x=2, y=67)

		self.main_screen_edit_text_button: customtkinter.CTkButton = customtkinter.CTkButton(master=self.main_screen_taskbar, text="уреди текст", text_color=text_color, height=20, width=295, corner_radius=5, fg_color=button_color, font=("Roman", 22), command=self.__edit_text_)
		...
		self.main_screen_open_button: customtkinter.CTkButton = customtkinter.CTkButton(master=self.main_screen_taskbar, text="отвори фајл", text_color=text_color, height=20, width=295, corner_radius=5, fg_color=button_color, font=("Roman", 22), command=self.__open__)
		...
		self.main_screen_converter_button: customtkinter.CTkButton = customtkinter.CTkButton(master=self.main_screen_taskbar, text="претварач", text_color=text_color, height=20, width=295, corner_radius=5, fg_color=button_color, font=("Roman", 22), command=self.__convert__)
		...
		self.main_screen_code_editor_button: customtkinter.CTkButton = customtkinter.CTkButton(master=self.main_screen_taskbar, text="редактор кода", text_color=text_color, height=20, width=295, corner_radius=5, fg_color=button_color, font=("Roman", 22), command=self.__code_editor__)
		...
		self.main_screen_settings_button: customtkinter.CTkButton = customtkinter.CTkButton(master=self.main_screen_taskbar, text="подешавања", text_color=text_color, height=20, width=295, corner_radius=5, fg_color=button_color, font=("Roman", 22), command=self.__settings__)
	...
	def __save_text_as_docx__(self) -> None:
		try:
			...
			self.font.size: Pt = Pt(int(self.main_screen_edit_size_button_data))
			if self.main_screen_edit_color_button_data == "black":
				self.font.color.rgb: RGBColor = RGBColor(0, 0, 0)

			elif self.main_screen_edit_color_button_data == "white":
				self.font.color.rgb: RGBColor = RGBColor(255, 255, 255)

			elif self.main_screen_edit_color_button_data == "red":
				self.font.color.rgb: RGBColor = RGBColor(250, 0, 0)

			elif self.main_screen_edit_color_button_data == "green":
				self.font.color.rgb: RGBColor = RGBColor(0, 255, 0)

			else:
				self.font.color.rgb: RGBColor = RGBColor(0, 0, 255)
	...
	def __open_file_docx__(self) -> None:
		try:
			self.openned_file: docx.Document = docx.Document(filedialog.askopenfilename(title="open docx file", filetypes=[("Word file (*.docx)", "*.docx")], defaultextension=[("Word file (*.docx)", "*.docx")]))
	...
	def __pdf_to_docx__(self) -> None:
		try:
			self.file: str = filedialog.askopenfilename(title="convert pdf file", filetypes=[("Pdf file (*.pdf)", "*.pdf")], defaultextension=[("Pdf file (*.pdf)", "*.pdf")])
			self.converted_file: aspose_words.Document = aspose_words.Document(self.file)
	...
	def __docx_to_pdf__(self) -> None:
		try:
			self.file: str = filedialog.askopenfilename(title="convert docx file", filetypes=[("Word file (*.docx)", "*.docx")], defaultextension=[("Word file (*.docx)", "*.docx")])
			self.converted_file: aspose_words.Document = aspose_words.Document(self.file)
	...
	def __txt_to_pdf__(self) -> None:
		try:
			self.file: str = filedialog.askopenfilename(title="convert txt file", filetypes=[("Text file (*.txt)", "*.txt")], defaultextension=[("Text file (*.txt)", "*.txt")])
			self.converted_file: aspose_words.Document = aspose_words.Document(self.file)
			self.converted_file.save(self.file + ".pdf")
	...
	def __word_count__(self, event: str) -> None:
		self.main_screen_frame_textbox_data = self.main_screen_frame_textbox.get("0.0", tkinter.END)
	...
	def __open_terminal__(self, event: str) -> None:
		try:
			from My_Diary_command_prompt import Terminal
		
			self.terminal_frame: Terminal = Terminal()
		
		except ImportError:
			if language_data == "Српски":
				showerror(title="Грешка", message="Нема те фајл са терминалом")

			elif language_data == "English":
				showerror(title="Error", message="You don't have terminal file")

			else:
				showerror(title="Ошибка", message="У вас нет файла с терминалом")
	...
	def __exit__(self) -> None:
		if language_data == "Српски":
			self.main_screen_exit: askyesno = askyesno(title="излаз", message="желите да изађете?")
			if self.main_screen_exit: sys.exit()

		elif language_data == "English":
			self.main_screen_exit: askyesno = askyesno(title="exit", message="would you like to exit?")
			if self.main_screen_exit: sys.exit()

		else:
			self.main_screen_exit: askyesno = askyesno(title="выход", message="желайте выйти?")
			if self.main_screen_exit: sys.exit()
